...
class OpenAICompatibleProvider(BaseProvider):
    ...
    def _get_retry_request_body(self, error: Exception, body: dict) -> dict | None:
        ...
        cap = self._extract_max_tokens_cap(error_text)
        if cap is None:
            return None

        current_max_tokens = body.get("max_tokens")
        if not isinstance(current_max_tokens, (int, float)):
            return None

        current_max_tokens_int = int(current_max_tokens)
        if current_max_tokens_int <= cap:
            return None

        retry_body = dict(body)
        retry_body["max_tokens"] = cap
        logger.warning(
            "{}_STREAM: retrying with max_tokens={} after 400 cap error (was {})",
            self._provider_name,
            cap,
            current_max_tokens_int,
        )
        return retry_body

    @staticmethod
    def _extract_max_tokens_cap(error_text: str) -> int | None:
        """Parse an upstream max_tokens validation error and return the cap."""
        lowered = error_text.lower()
        if "max_tokens" not in lowered:
            return None

        matches = re.findall(
            r"(?:less than or equal to|maximum value for max_tokens is)\s*(\d+)",
            lowered,
        )
        if not matches:
            return None

        caps = [int(match) for match in matches if int(match) > 0]
        if not caps:
            return None
        return min(caps)
